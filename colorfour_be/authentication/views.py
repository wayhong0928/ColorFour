...
class LineLoginCallback(APIView):
  def get(self, request, *args, **kwargs):
    ...
    logger.debug(f'Received code: {code}, state: {state}, session state: {session_state}')
    
    if not state or state != session_state:
      logger.error('Invalid state: Received state does not match session state')
      return HttpResponseRedirect(f'{settings.FRONTEND_URL}/?loginResult=failure&message=Invalid state')

    if code:
      try:
        user_info = self.authenticate_with_line(code, settings.LINE_LOGIN_CALLBACK_URL)
        logger.debug(f'User info: {user_info}')
        user, created = self.create_or_get_user(user_info)
        if created:
          return HttpResponseRedirect(f'{settings.FRONTEND_URL}/profile-setup')
        return HttpResponseRedirect(f'{settings.FRONTEND_URL}/?loginResult=success')
      except Exception as e:
        logger.error(f'Error during Line login: {e}')
        return HttpResponseRedirect(f'{settings.FRONTEND_URL}/?loginResult=failure&message={str(e)}')
    return HttpResponseRedirect(f'{settings.FRONTEND_URL}/?loginResult=failure&message=No code provided')
  ...
  def authenticate_with_line(self, code, redirect_uri):
    ...
    if response.status_code != 200:
      ...
      raise Exception(f'Failed to obtain access token from LINE: {response.content}')

    access_token = response.json().get('access_token')
    if not access_token:
      logger.error('Failed to obtain access token from LINE: No access token received')
  ...
  def create_or_get_user(self, user_info):
    ...
    profile, created = UserProfile.objects.get_or_create(user=user, defaults={'line_id': user_info['userId']})
    if created:
...
class GoogleLoginCallback(APIView):
  def get(self, request, *args, **kwargs):
    ...
    if code:
      try:
        user_info = self.authenticate_with_google(code, settings.GOOGLE_LOGIN_CALLBACK_URL)
        logger.debug(f'User info: {user_info}')
        user, created = self.create_or_get_user(user_info)
        if created:
          return HttpResponseRedirect(f'{settings.FRONTEND_URL}/profile-setup')
  ...
  def authenticate_with_google(self, code, redirect_uri):
    ...
    if response.status_code != 200:
      logger.error(f'Failed to obtain access token from Google: {response.content}')
      raise Exception(f'Failed to obtain access token from Google: {response.content}')

    access_token = response.json().get('access_token')
    if not access_token:
      logger.error('Failed to obtain access token from Google: No access token received')
  ...
  def get_google_user_info(self, access_token):
    ...
    if response.status_code != 200:
      ...
      raise Exception(f'Failed to obtain user info from Google: {response.content}')
  ...
  def create_or_get_user(self, user_info):
    user, created = User.objects.get_or_create(
      ...
      defaults={'first_name': user_info['given_name'], 'last_name': user_info['family_name'], 'email': user_info['email']}
    )
    profile, created = UserProfile.objects.get_or_create(user=user, defaults={'google_id': user_info['id']})
    if created:
      profile.is_new_user = True
      profile.save()
    return user, created
