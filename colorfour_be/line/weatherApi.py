    ...
    if conversation_state["step"] is None:
        ...
        if mtext == "縣市":
            ...
            line_bot_api.reply_message(
                event.reply_token, TextSendMessage(text="請輸入縣市")
            )
        elif mtext == "景點":
        ...
        line_bot_api.reply_message(
            event.reply_token, TextSendMessage(text="請輸入區域")
        )
    ...
    try:
        ...
        for url in urls:
            ...
            for station in stations:
                if (
                    station["GeoInfo"]["CountyName"] == city
                    and station["GeoInfo"]["TownName"] == district
                ):
...
def get_place_coordinates(spot_name):  # 景點經緯度位置
    try:
        ...
        params = {"address": spot_name, "key": api_key, "language": "zh-TW"}

        response = requests.get(base_url, params=params)
        if response.status_code != 200:
            raise Exception(
                f"Error fetching data from Google Maps API, status code: {response.status_code}"
            )
...
def get_weather_by_coordinates(lat, lon):  # 景點天氣
    try:
        ...
        if response.status_code != 200:
            raise Exception(
                f"Error fetching data from CWB API, status code: {response.status_code}"
            )
