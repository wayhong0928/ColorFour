    ...
    <main>
      <section class="container my-5">
        ...
        <div class="button-group mb-4">
          ...
          <button @click="enableCustomName" class="btn btn-outline-secondary mx-1">自訂名稱</button>
        </div>

        <!-- 自訂名稱區域 -->
        <div v-if="customName" class="mb-4">
          <label for="custom-name" class="form-label">自訂名稱</label>
          <input type="text" id="custom-name" v-model="customInput" class="form-control" placeholder="輸入自訂名稱" />
        ...
        <div class="d-flex justify-content-between">
          ...
          <button class="btn btn-primary" @click="startTest" :disabled="loading">前往測驗</button>
        </div>
      </section>
  ...
  export default {
    ...
    methods: {
      toggleSelection(name) {
        ...
        if (index === -1) this.selectedNames.push(name);
        else this.selectedNames.splice(index, 1);
      },
      enableCustomName() {
